...
from Time_Track.datasets.data_prefetcher_sub import data_prefetcher
...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for _ in metric_logger.log_every(range(len(data_loader)), print_freq, header):
        ...
        for i,img in enumerate(samples_sub):
            ...
            plt.imshow(img)
            plt.savefig('w_input/input_{}.png'.format(i))
        """
...
#     metric_logger.add_meter('class_error', utils.SmoothedValue(window_size=1, fmt='{value:.2f}'))
    ...
    for samples, targets in metric_logger.log_every(data_loader, 10, header):
        ...
        samples.tensors = samples.tensors.type(tensor_type)
        #print('eval data = ,',samples.tensors.shape)
        samples.mask = samples.mask.type(tensor_type)
        ...
        with torch.cuda.amp.autocast(enabled=fp16):
            if det_val:
                ...
                if count == 1:
                    pre_samples = samples
                    outputs, pre_embed = model(samples,pre_samples, pre_embed)
                    count += 1
                    pre_samples = samples
